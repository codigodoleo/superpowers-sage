...
if (existsSync(skillsDir)) {
  for (const entry of readdirSync(skillsDir, { withFileTypes: true })) {
    ...
    if (!fm.name.startsWith('sage:') && !fm.name.startsWith('wp-') && !fm.name.startsWith('roots-')) {
      log('✗', `${label} — name "${fm.name}" must start with "sage:", "wp-", or "roots-"`);
      errors++;
      continue;
    }
...
if (versions.Claude && versions.Cursor && versions.Claude !== versions.Cursor) {
  log('✗', `Version mismatch: Claude=${versions.Claude}, Cursor=${versions.Cursor}`);
  errors++;
}
