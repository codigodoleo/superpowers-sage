...
| **Architecting** | `/architecting` | Brainstorming + ADR + design capture + content modeling |
...
```
/architecting  →  (generates plan + assets)
...
| Base Skill | Used By |
|---|---|
| `brainstorming` | `/architecting` (core engine) |
| `writing-plans` | `/architecting` (plan generation) |
